...
**Honest gap:** Written from training knowledge, not verified live at the time -- since confirmed working end-to-end against the real API by the project owner on 2026-07-23 (after fixing the auth method -- see the `x-goog-api-key` header entry below -- and using `gemini-2.5-flash`/`text-embedding-001` instead of the original guessed model names).
...
**Honest gap:** Both are reasonable defaults, not calibrated values. The LLM reranker has since been exercised against a real live question ("where is my refund" scored the correct top document at confidence 1.0) -- still worth tuning both constants against a broader set of real questions before treating them as settled.
...
**Honest gap:** Confirmed working end-to-end against the live API on 2026-07-23 (passed on the first attempt for a well-covered question, no retry needed). A dedicated NLI model would likely be more reliable for this specific judgment in a non-serverless deployment; this is an infrastructure-driven choice, not a universal preference (mirroring the existing DECISIONS.md entry on the hosted-embedding-API choice). Still untested: whether it can actually *detect* a real contradiction and trigger a retry against the live API (the local-fallback tests can only simulate this via an injected fake, per the entry below).
...
### retrieval.py/graph.py: Gemini API key sent via x-goog-api-key header, not ?key= query param (bug fix)
**Why:** All four Gemini calls (embedding, rerank, synthesis, guardrail) originally sent the API key as a `?key=` query parameter, which is documented as the classic/legacy method. Live testing found this failed with a 404 for the user's actual API key (a newer-format key starting with `AQ.` rather than the classic `AIzaSy...` format) -- confirmed fixed by testing the same request in Postman with the key moved to an `x-goog-api-key` header instead, which succeeded. Google appears to be transitioning to this newer key format across accounts, and the query-param auth path doesn't accept it, while the header does.
**Honest gap:** None functionally -- this is a corrected implementation detail, not a design trade-off. Still unverified whether the header approach is universally preferred going forward for all key formats (including classic `AIzaSy` keys) or specifically required for the newer format; the header approach works for both per Google's API docs, so there's no reason to special-case by key format.

### Tax Help Assistant: full pipeline confirmed working end-to-end against the live deployed API (2026-07-23)
**Why:** After the model-name and auth-header fixes above, the project owner tested `POST /help/ask` with `{"question": "where is my refund"}` against the live Vercel deployment and got back a grounded, non-hallucinated answer (`answered: true`, paraphrasing `faq-check-status-no-account` rather than inventing anything), 5 sources, `confidence: 1`. This is the first live confirmation that every real backend in this feature actually works together: hosted embeddings, BM25/RRF fusion, LLM reranking, Gemini synthesis, and the guardrail check, none of which could be exercised from the sandboxed dev environment this was built in.
**Honest gap:** Only the happy path has been confirmed live so far. Still unverified against the real API: the confidence-gate path (an actual out-of-corpus question), the guardrail genuinely rejecting a real generated answer and triggering a retry, and the graceful-degradation paths (all currently only proven via injected fakes in test_integration_help_assistant.py, per that file's own module docstring).

### storage.py: two more seed returns added for the frontend build (paper-filed, SENT)
**Why:** DEMO_WALKTHROUGH_SCRIPT.md's narration covers a paper-filed "no status yet" moment and a terminal SENT state, neither of which the original 3-return seed set (deliberately minimal for the 6-row acceptance table) included. The frontend's scenario switcher needed real, distinct examples of both to demo them honestly rather than describe them without evidence.
**Honest gap:** The paper-filed return uses `status_code=RECEIVED` rather than a genuine "no status at all" state -- see the dedicated entry below for why, and what's lost by that compromise.

### storage.py: paper-filed scenario uses status_code=RECEIVED, not a new "no status yet" value
**Why:** Real IRS behavior for paper returns is closer to "no tracking information exists at all for weeks," but `03_API_CONTRACT.yaml`'s `status_code` enum is fixed at `[RECEIVED, APPROVED, SENT, NO_REFUND_PENDING]` -- adding a 5th value would be a real, unflagged contract deviation, not a demo-only addition. `RECEIVED` is honestly true (the IRS has logged the return), so the UI carries the "paper takes weeks longer, this is expected" framing based on `filing_method == PAPER` rather than the backend inventing a status the contract doesn't declare.
**Honest gap:** This is a pragmatic compromise, not a perfect model of reality -- a real paper-filed return often shows *no* record at all for weeks, not a "received" one. Worth flagging explicitly if asked why paper filing doesn't get its own status code.

### api/index.py: two new demo-only endpoints (GET /demo/circuit-breaker-status, POST /demo/clear-cache)
**Why:** Neither exists in `03_API_CONTRACT.yaml`. The breaker-status endpoint exists because the task explicitly wants CLOSED/OPEN/HALF_OPEN "surfaced directly rather than only inferring from response behavior" -- there was no way to read breaker state from outside `irs_integration.py` before this. The clear-cache endpoint backs `05_ACCEPTANCE_CRITERIA.md`'s scenario 2 ("same return, forced cache miss"), which had no trigger mechanism before this either. Both follow the same "demo scaffolding, not part of the contract" pattern already established by `/demo/set-irs-mode` -- same router, same prefix, same reasoning.
**Honest gap:** None -- straightforward additions matching an existing, already-justified pattern.

### kv_store.py: added delete(), a real feature gap, not test-only tooling
**Why:** `KVStore` only exposed get/set/ttl -- there was no way to actually remove a key. `cache_layer.clear_cache()` (backing the demo's "force cache miss" button) needs to remove both the durable value and the freshness marker outright, not just let them expire naturally. Added to both backends (`_LocalBackend.delete()` via dict `.pop()`, `_UpstashBackend.delete()` via the Upstash client's own `delete()`) and folded into the shared `assert_kv_contract()` test helper so both backends are held to the same contract, same as every other KVStore method.
**Honest gap:** None.

### refund_status.py: paper-filed returns get no predicted_window, even mid-flow (bug fix, caught in-browser)
**Why:** Found while actually looking at the paper-filed scenario (`RET-2025-00004`) rendered in the frontend: a confident-looking delivery-window date range sat right next to the honest "paper returns take weeks, no detail yet" framing -- contradictory on screen even though each piece was independently correct. `_build_cached_data()` now suppresses `predicted_window` specifically for `FilingMethod.PAPER`, while `explanation` still runs normally (this return has no EITC/CTC flag, so it still correctly says "still processing").
**Honest gap:** Static code review and the original unit tests didn't catch this -- both pieces were individually correct, only their *combination* on one screen was the problem. Underscores why the system prompt's "test the UI in a browser, don't just claim success" instruction mattered here specifically.

### static/index.html: customer-facing errors go through friendlyErrorMessage(), never the raw `detail` string (bug fix, caught in-browser)
**Why:** `RefundDataUnavailableError`'s technical detail (e.g. `"outcome=CONNECTION_FAILURE"`) was originally rendered directly in the error card -- a real violation of CLAUDE.md's "honest language empathizing with customers," since a customer has no use for an internal outcome enum. `friendlyErrorMessage(errorCode)` now maps the backend's stable `error` code field (`return_not_found`, `invalid_tax_year`, `refund_data_unavailable`) to calm, specific customer-facing copy, falling back to a generic apology for anything unmapped -- keyed off the code, never the free-text `detail` string, so it can't accidentally leak internals in the future either.
**Honest gap:** None -- this is a straightforward customer-facing copy fix once found; no logic change.

### app/cache_layer.py: clear_cache() only deletes the freshness marker, not the durable value (bug fix, caught while rehearsing the demo flow)
**Why:** The original implementation deleted both KV keys, which meant the demo's "force cache miss" button also destroyed the exact stale-fallback data the circuit-breaker-open scenario needs to serve. Every rehearsal of "force miss, then fail 3 times" hit the no-fallback-exists edge case (`RefundDataUnavailableError`) instead of the intended "serve stale cache, marked stale" moment -- the single most demo-critical path in the whole system, per `05_ACCEPTANCE_CRITERIA.md`. Fixed to delete only `_fresh_key`, matching what "force cache miss" actually means: pretend the TTL lapsed, don't erase history.
**Honest gap:** None -- this was a genuine bug, not a trade-off; `test_clear_cache_forces_a_miss_but_preserves_the_stale_fallback` now asserts the corrected behavior directly.

### static/index.html: circuit breaker indicator polls immediately after checkStatus(), not only on a 4s interval (bug fix, caught in-browser)
**Why:** The breaker indicator originally only refreshed on a fixed 4-second timer, so clicking "Check Status" repeatedly during the FAILING-mode demo didn't visibly flip to OPEN until up to 4 seconds later -- undercutting exactly the moment `05_ACCEPTANCE_CRITERIA.md` calls "the single most technically impressive live moment." Added an explicit `pollBreakerState()` call in `checkStatus()`'s `finally` block so the indicator updates in the same round trip as the status check that actually tripped the breaker; the 4s interval stays as a backstop for state changes not driven by the user's own clicks (e.g. HALF_OPEN's cooldown expiring while they're reading the screen).
**Honest gap:** None -- purely additive responsiveness fix, the polling endpoint itself already existed.

### vercel.json: added a `@vercel/static` build for `static/**`, and a `/` → `/static/index.html` route (bug fix)
**Why:** `vercel.json`'s `builds` array only ever declared `api/index.py` (`@vercel/python`) -- the `/static/(.*)` route rule existed, but with nothing building `static/` into deployable output, Vercel had no file to serve there, so both `/static/index.html` and the bare root returned `{"detail": "Not Found"}` (FastAPI's own 404, meaning the request fell through to `api/index.py` and matched no route). Adding a `@vercel/static` build entry for `static/**` makes those files actual deployment output. Also added a `/` → `/static/index.html` route so the deployment's root URL itself is a working entry point, not just `/static/index.html`.
**Honest gap:** This was only caught because the project owner tried the live Vercel URL directly -- `TestClient`-based verification (used earlier for `/docs` and `/openapi.json`) can't catch a `vercel.json` build-config gap like this one, since it runs the ASGI app directly and never exercises Vercel's own static-build pipeline. Confirms the "some phases genuinely can only be checked live" caveat already called out for the KV dependency.
