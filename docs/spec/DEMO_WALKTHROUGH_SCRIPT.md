...
**3c. Approved, but past its own predicted window (Karen, `RET-2025-00006`)**
> "This one's interesting: status is Approved, there's a refund amount, but watch what's *not* here — no delivery window. This return's estimated window came and went weeks ago and the refund still hasn't shipped. Rather than keep showing a date range that's now obviously wrong — which would look either broken or dishonest — the system recognizes the window has elapsed and switches to plain language: 'Your refund is taking a little longer than typical — this happens and doesn't mean anything is wrong.'" *(worth pausing here)* "This is the same honesty principle as 'still processing,' just applied to a prediction instead of an explanation — I'd rather say 'we don't know exactly when anymore' than show a stale estimate as if it were still current."
